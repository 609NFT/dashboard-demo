...
// A short trailing trend for the stat-card sparklines. `slope` gently tilts the
// series so growth metrics rise and churn drifts down, with small daily noise so
// it reads as real data, not a straight line.
function spark(seedBase, base, amp, slope) {
  const today = dayBucket();
  const out = [];
  for (let i = 6; i >= 0; i--) {
    const r = seededRand(seedBase + (today - i));
    const t = (6 - i) / 6;
    out.push(Math.round(base + slope * t + (r() - 0.5) * amp));
  }
  return out;
}
...
function stats() {
  ...
  // Slope the sparkline in the direction of the day-over-day delta so the line's
  // shape and its (green/red) colour always agree with the badge.
  const dir = (delta) => (delta >= 0 ? 1 : -1);
  const mrrD = pctDelta(mrr, mrrPrev);
  const sigD = pctDelta(signups, signupsPrev);
  const actD = pctDelta(active, activePrev);
  const chrD = pctDelta(churn, churnPrev);

  return [
    { label: 'MRR', value: `$${mrr.toLocaleString()}`, delta: mrrD, unit: '$', spark: spark(11, 55, 13, dir(mrrD) * 30) },
    { label: 'Signups (24h)', value: signups, delta: sigD, spark: spark(23, 50, 20, dir(sigD) * 30) },
    { label: 'Active users', value: active.toLocaleString(), delta: actD, spark: spark(37, 55, 12, dir(actD) * 28) },
    { label: 'Churn (30d)', value: `${churn.toFixed(2)}%`, delta: chrD, invert: true, spark: spark(51, 58, 12, dir(chrD) * 26) },
  ];
}
...
// A believable SaaS growth curve: a steady upward trend, a gentle weekend dip,
// and small day-scale noise — instead of pure random draws that spiked into
// jagged Vs. `days` selects the trailing window (7/30/90); the trend is anchored
// to a fixed 90-day origin so shorter ranges are just the recent slice of the
// same curve, not a different shape.
function activitySeries(days = 30) {
  const span = 90;
  const n = Math.max(1, Math.min(span, parseInt(days, 10) || 30));
  const points = [];
  for (let i = n - 1; i >= 0; i--) {
    const date = new Date(Date.now() - i * 86400000);
    const t = (span - i) / span; // 0..1 position along the growth curve
    const dow = date.getDay();
    const weekend = dow === 0 || dow === 6 ? 0.82 : 1;
    const rr = seededRand(9000 + Math.floor(date.getTime() / 86400000));
    const rs = seededRand(4200 + Math.floor(date.getTime() / 86400000));

    const revTrend = 3900 + t * 3300;
    const revenue = Math.round((revTrend + (rr() - 0.5) * 520) * weekend);
    const sigTrend = 32 + t * 34;
    const signups = Math.max(6, Math.round((sigTrend + (rs() - 0.5) * 12) * weekend));

    points.push({ date: date.toISOString().slice(0, 10), revenue, signups });
  }
  return points;
}
...
function notifications() {
  return [
    { id: 'n1', type: 'signup', title: 'New signup', body: 'Riley Kim joined on the Pro plan.', ago: '12m', unread: true },
    { id: 'n2', type: 'payment', title: 'Payment received', body: '$240 from Northwind for the Team plan.', ago: '1h', unread: true },
    { id: 'n3', type: 'churn', title: 'Churn risk', body: '2 Team accounts went past due this week.', ago: '3h', unread: true },
    { id: 'n4', type: 'report', title: 'Weekly report ready', body: 'Your metrics summary for last week is in.', ago: '1d', unread: false },
  ];
}

module.exports = { stats, activitySeries, users, revenue, notifications };
